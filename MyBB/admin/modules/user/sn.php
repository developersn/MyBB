...
$page->add_breadcrumb_item("پرداخت آنلاين پرداخت آنلاین", "index.php?module=user-sn");
	...
	$page->output_header("بسته های عضویت آنی (پرداخت آنلاین)");

	$sub_tabs['packs'] = array(
		'title' => 'بسته های عضویت آنی (پرداخت آنلاین)',
		'link' => "index.php?module=user-sn",
		'description' => 'در این بخش می توانید بسته های عضویت ویژه را مدیریت کنید.'
	);

	$sub_tabs['add_pack'] = array(
		'title' => 'افزودن بسته ی تازه',
		'link' => "index.php?module=user-sn&amp;action=add_pack",
...
if($mybb->input['action'] == "edit_pack")
{
	$sub_tabs['edit_pack'] = array(
		'title' => 'ویرایش بسته',
		'link' => "index.php?module=user-sn&amp;action=edit_pack",
	...
	$sub_tabs['manual_add'] = array(
		...
		'link' => "index.php?module=user-sn&amp;action=manual_add",
	...
	$sub_tabs['tractions'] = array(
		...
		'link' => "index.php?module=user-sn&amp;action=tractions",
...
$d = "سال";
		...
		$pname=$over['title'];
		$table->construct_cell("<strong><a href=\"index.php?module=user-sn&amp;action=edit_pack&amp;num={$over['num']}\">{$over['title']}</a></strong></td><td>{$over['description']}</td><td>{$over['period']} {$d}  </td><td>{$over['price']} تومان</td>");
		$popup = new PopupMenu("sub_{$over['num']}", $lang->options);
		$popup->add_item($lang->delete, "index.php?module=user-sn&amp;action=delete_pack&amp;num={$over['num']}");
		$popup->add_item($lang->edit, "index.php?module=user-sn&amp;action=edit_pack&amp;num={$over['num']}");
	...
	$querypage = $db->simple_select("sn", "COUNT(num) as packs", "1=1");
	$total_rows = $db->fetch_field($querypage, "packs");
			
	$pagination = draw_admin_pagination($mybb->input['page'], $per_page, $total_rows, "index.php?module=user-sn&amp;page={page}");
	echo $pagination;					
	...
	if(!$mybb->input['group'])
{
	...
	if(is_numeric($mybb->input['packprice'])&&floatval($mybb->input['packprice'])<100)
{
			$errors[] = "حد‌اقل قیمت بسته با توجه به قوانین پرداخت آنلاین باید 100 تومان باشد.";
}
...
$db->insert_query("sn", $insert_query);
					flash_message('بسته ی مورد نظر با موفقیت ذخیره شد', 'success');
					admin_redirect("index.php?module=user-sn");
					}
}
...
// Time
		...
		$form = new Form("index.php?module=user-sn&amp;action=add_pack", "post");
		if($errors)
	{
	...
	$form_container->output_row("هزینه عضویت (به تومان) <em>*</em>", "", $form->generate_text_box('packprice', $mybb->input['packprice'], array('id' => 'packprice')), 'packprice');
...
'period' => intval($mybb->input['period']),
					...
					$db->update_query('sn', $update_array, 'num=\''.intval($num).'\'');
...
//
    ...
    $num = $mybb->input['num'];
	$query = $db->simple_select("sn","*","num='$num'");
    $edit = $db->fetch_array($query);

	$form = new Form("index.php?module=user-sn&amp;action=edit_pack&amp;num={$num}", "post");
			if($errors)
	{
		$page->output_inline_error($errors);
	}
	...
	$form_container->output_row("هزینه عضویت (به تومان) <em>*</em>", "", $form->generate_text_box('packprice', $edit['price'], array('id' => 'packprice')), 'packprice');
...
{
$num=intval($mybb->input['num']);
$db->query("DELETE FROM ".TABLE_PREFIX."sn WHERE num=$num");
	...
	if($mybb->input['action'] == "tractions")
{
	...
	if($mybb->input['uname'])
	{
		...
		if(!$user['uid'])
		{
			...
			admin_redirect("index.php?module=user-sn&action=tractions");
		}
...
//\End Time Filter
	...
	//\Filter
		...
		$table->construct_header("گزینه‌ها", array('class' => 'align_left', width => '7%'));
			...
			$query4 = $db->simple_select('sn', 'num, title', '', array('order_by' => 'num', 'order_dir' => 'asc'));
			while($pack = $db->fetch_array($query4, 'num, title'))
			{
				$packs[$pack['num']] = $pack['title'];
			}
		$query5 = $db->query("SELECT l.*
		FROM ".TABLE_PREFIX."sn_tractions l
...
if($track['stauts'] != 0)
{
		$table->construct_cell("{$track['tid']}</td><td>{$profile_link}</td><td>{$track['payed']} تومان</td><td>{$packs[$pid]}</td><td>{$track['trackid']}</td><td>{$stdate}</td><td>{$expdate}</td><td>{$st}</td><td><a href=\"index.php?module=user-sn&amp;action=end&amp;tid={$track['tid']}\">پایان دادن</a></td>");
				$table->construct_row();
				}
else{
		$table->construct_cell("{$track['tid']}</td><td>{$profile_link}</td><td>{$track['payed']} تومان</td><td>{$packs[$pid]}</td><td>{$track['trackid']}</td><td>{$stdate}</td><td>{$expdate}</td><td>{$st}</td><td><a href=\"index.php?module=user-sn&amp;action=deletetrac&amp;tid={$track['tid']}\">حذف</a></td>");
				$table->construct_row();
	...
	$querypage = $db->simple_select("sn_tractions l", "COUNT(l.tid) as tracs", "1=1 {$additional_sql_criteria}");
	$total_rows = $db->fetch_field($querypage, "tracs");
	$pagination = draw_admin_pagination($mybb->input['page'], $per_page, $total_rows, "index.php?module=user-sn&amp;action=tractions&amp;page={page}{$additional_criteria}");
				echo $pagination;
				echo "<div class = \"float_left\"><a href=\"index.php?module=user-sn&amp;action=deletealltrac\">حذف تمامی تراکنش‌های پایان یافته</a></div>";
	...
	$packquery = $db->simple_select("sn", "num,title", "1=1");
	...
	$neper['1']="قبل";
    $neper['2']="بعد";
	$form = new Form("index.php?module=user-sn&amp;action=tractions", "post");
...
$query = $db->simple_select("sn_tractions", "*", "tid = $tid");
  ...
  'usergroup' => $pgid
                ...
                $db->update_query("sn_tractions", $update_array, "`tid` = '$tid'");
...
{
$tid=intval($mybb->input['tid']);
$db->query("DELETE FROM ".TABLE_PREFIX."sn_tractions WHERE tid=$tid");
...
{
$db->query("DELETE FROM ".TABLE_PREFIX."sn_tractions WHERE stauts=0");
...
$uid = $user['uid'];
$pgid = $user['usergroup'];
$queryp = $db->simple_select("sn", "*", "num={$packid}");
...
$querychk = $db->simple_select("sn_tractions", "stauts", "uid='{$uid}'");
...
//Add TR
$db->insert_query("sn_tractions", $insert_query);
//Change UG
if ($db->table_exists("bank_pey") && $bank != 0)
{
    ...
    'username' => "مدیریت",
    'time' => TIME_NOW,
     'info' => "خرید از درگاه پرداخت آنلاین",
);
...
//Message
	...
	$form = new Form("index.php?module=user-sn&amp;action=manual_add", "post");
		if($errors)
	{
		$page->output_inline_error($errors);
	}
	...
	$form_container->output_row("شماره‌ی تراکنش:<em>*</em>", "شماره‌ی تراکنش پرداخت آنلاین را وارد کنید.", $form->generate_text_box('‌trackid', $mybb->input['trackid'], array('id' => '‌trackid')), '‌trackid');
	$form_container->end();
			echo '
